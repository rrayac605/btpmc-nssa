package mx.gob.imss.cit.pmc.aju.reader;
...
@Component
@StepScope
public class InsertaArchivoAJUInicialItemReader extends FlatFileItemReader<RegistroDTO> {

	private final static Logger logger = LoggerFactory.getLogger(InsertaArchivoAJUInicialItemReader.class);
	...
	public void init() {
		...
		setLineMapper(lineMapperInsertaArchivoInicialAJU());
		try {
			setResource(new FileSystemResource(parametrosService
					...
					.concat((nombre != null && !nombre.trim().equals("")) ? nombre
							: Utils.obtenerNombreArchivoNSSA(IdentificadorArchivoEnum.ARCHIVO_AJU.getIdentificador()))));
		} catch (ParametroNoExisteException e) {
			logger.error(e.getMessage(), e);
		}
		setLinesToSkip(1);
		setSkippedLinesCallback(headerLineCallbackHandlerAJU);
		setEncoding("UTF-8");
	}

	@Bean
	@StepScope
	public LineMapper<RegistroDTO> lineMapperInsertaArchivoInicialAJU() {
		DefaultLineMapper<RegistroDTO> lineMapper = new DefaultLineMapper<RegistroDTO>();
		lineMapper.setLineTokenizer(productLineTokenizerArchivoInicialAJU());
		lineMapper.setFieldSetMapper(productFieldSetMapperArchivoInicialAJU());
		return lineMapper;
	}

	@Bean
	@StepScope
	public LineTokenizer productLineTokenizerArchivoInicialAJU() {
	...
	@Bean
	@StepScope
	public FieldSetMapper<RegistroDTO> productFieldSetMapperArchivoInicialAJU() {
